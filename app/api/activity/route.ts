import { fetchRecentActivity, RateLimitError } from "@/src/github-fetcher";
import { generateActivitySVG, generateErrorSVG, generateRateLimitSVG } from "@/src/svg-generator";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await fetchRecentActivity(username);
    const translations = getTranslations(locale || "en");
    const svg = generateActivitySVG(data, themeObj, translations, custom_css, font);
